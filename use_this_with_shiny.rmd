...
Use the interactive button on the left to view monthly patterns in taxon-specific groups that iNaturalist has identified as "iconic taxa", such as *Arachnida* (spiders), *Aves* (birds), *Fungi* (fungi), and *Insecta* (insects).
...
```{r}
selectInput("iconic_taxon_name",
            label = "Taxonomic Group:",
            choices = c("Plantae", "Insecta", "Arachnida", "Aves", "Actinopterygii", "Fungi", "Protozoa", "Mollusca", "Mammalia", "Reptilia", "Amphibia"),
            selected = "Plantae")
...
renderPlot({
  ggplot(inat_time %>%
           dplyr::filter(iconic_taxon_name == input$iconic_taxon_name),
       aes(x = months1,
           y = count)) +
  geom_smooth(aes(color = input$iconic_taxon_name),
              se = F) +
  theme_classic()+ 
  geom_point(aes(color = input$iconic_taxon_name)) +
  labs(x = "Month",
       y = "Monthly Observations",
       color = "Taxonomic Group")
})
